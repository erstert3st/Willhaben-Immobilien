...
class WillhabenQueryBuilder:
    ...
    def __init__(self):
        ...
        self.property_types: Optional[List[PROPERTY_TYPE]] = (
            []
        )  # Immobilientyp. if none is selected, the result will contain all types
        self.free_area_types: Optional[List[FREE_AREA_TYPE]] = (
            []
        )  # Freiflächen. if none is selected, the result will contain all types
        self.estate_preferences: Optional[List[ESTATE_PREFERENCE]] = (
            []
        )  # Ausstattung. if none is selected, the result will contain all types
    ...
    def add_number_of_rooms(self, num: Literal["1", "2", "3", "4", "5", "6-9", "10+"]):
        if num not in ["1", "2", "3", "4", "5", "6-9", "10+"]:
            raise ValueError(
                "Invalid number of rooms. Must be 1, 2, 3, 4, 5, 6-9 or 10+ in datatype str."
            )
        if num in ["1", "2", "3", "4", "5"]:
    ...
    def add_property_type(
        self, property_type: Union[PROPERTY_TYPE, List[PROPERTY_TYPE]]
    ):
...
#lol make bugfix on there git xD 
                ...
                if attribute["name"] == "PUBLISHED_String":
                    ...
                    try:
                        published_date = datetime.strptime(
                            published_date_str, date_format
                        ).isoformat()
                    except ValueError:
